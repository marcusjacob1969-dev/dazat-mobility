...
try {
  ...
  // Phase 0.137: provider-disabled finance exposes no capture/charge/refund mutation surface.
  for (const path of [
    ...
    const response = await post(path, tokens.actor, {}, `phase-0-137-${path.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}`);
    assert.equal(response.statusCode, 404, `Unexpected finance mutation surface at ${path}: ${response.body}`);
  }
  const financeRoutesSource = readFileSync(new URL('../services/api/src/modules/finance/routes.ts', import.meta.url), 'utf8');
  const financeServiceSource = readFileSync(new URL('../services/api/src/modules/finance/finance-service.ts', import.meta.url), 'utf8');
  for (const forbidden of ['capturePayment', 'chargePayment', 'refundPayment', 'providerCapture', 'providerCharge', 'providerRefund']) {
    assert.equal(financeRoutesSource.includes(forbidden), false, `Forbidden provider mutation surface found in finance routes: ${forbidden}`);
  ...
  // Phase 0.120: route parameter validation must fail before any journey lookup.
  ...
  expectCode(await get('/v1/control-room/fatigue-handovers/' + ids.handover + '/bookings/' + ids.incidentBooking + '/core-journey-progress?unexpected=1', tokens.operator), 200);
...
\n  // Phase 0.143: terminal Payment states cannot be rewound into earlier lifecycle states.
  ...
  await assertDatabaseRejects(
    "UPDATE finance.payment SET status = 'AUTHORISED' WHERE id = $1",
    [ids.completedPayment]
  );
