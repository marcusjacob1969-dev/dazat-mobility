...
if (scripts['verify:phase-0-94'] !== 'node scripts/verify-phase-0-94.mjs') {
  errors.push('Root package must expose the Phase 0.94 verifier');
}
...
} else if (Number(currentRange[1]) !== 25 || Number(currentRange[2]) < 94) {
  errors.push('Current-checkpoint verifier must cover every checkpoint from Phase 0.25 through at least Phase 0.94');
}
if (errors.length) {
