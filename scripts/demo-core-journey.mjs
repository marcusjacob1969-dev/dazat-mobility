...
const scenarios = [
  ['happy-path', valid],
  ['ineligible-driver', { ...valid, driver: { ...valid.driver, complianceStatus: 'SUSPENDED' } }],
  ...
  ['stale-destination-evidence', { ...valid, completion: { ...valid.completion, destinationEvidenceAccepted: false } }]
];
...
assert.deepEqual(results.slice(1).map((result) => result.blocker), [
  ...
  'PICKUP_EVIDENCE_TOO_OLD',
  'RIDECHECK_MISMATCH',
  'ACTIVE_COMPLETION_HOLD',
  'DESTINATION_EVIDENCE_REQUIRED'
]);
