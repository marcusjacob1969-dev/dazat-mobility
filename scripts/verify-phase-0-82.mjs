...
const errors = [];
for (const truth of ['Smoke-test production API image', "docker image inspect --format '{{.Config.User}}'", 'docker run --detach --name dazat-api-smoke', '--retry-all-errors', '/health/live', '/v1/build-info', "engineering-phase-0.82", "trap 'docker logs dazat-api-smoke || true; docker rm --force dazat-api-smoke || true' EXIT"]) {
