...
const errors = [];
for (const truth of ['phase <= 67', "apps/control-room/tsconfig.json", "['run', 'test:api-runtime']", "['run', 'demo:core-journey']"]) if (!runner.includes(truth)) errors.push(`Current verification aggregate missing: ${truth}`);
