...
update('apps/rider/App.tsx', (source) => {
  let next = source;
  if (!next.includes("./src/journey-ui")) {
    next = next.replace("import { readCoreJourneyProgress } from './src/core-journey-api';", "import { readCoreJourneyProgress } from './src/core-journey-api';\nimport { presentRiderJourney } from './src/journey-ui';");
  }
  if (!next.includes('const journeyPresentation = coreJourneyProgress')) {
  ...
  if (!next.includes('accessibilityLabel="Rider canonical Journey projection"')) {
...
update('apps/driver/App.tsx', (source) => {
  let next = source;
  if (!next.includes("./src/journey-ui")) {
    next = next.replace("import { readDriverCoreJourneyProgress } from './src/core-journey-api';", "import { readDriverCoreJourneyProgress } from './src/core-journey-api';\nimport { presentDriverJourney } from './src/journey-ui';");
  }
  if (!next.includes('const journeyPresentation = coreJourneyProgress')) {
  ...
  if (!next.includes('accessibilityLabel="Driver canonical Journey projection"')) {
...
update('apps/control-room/src/App.tsx', (source) => {
  let next = source;
  if (!next.includes("presentCoreJourneyForSurface")) {
    next = next.replace("import { dazatTokens } from '@dazat/design-system';", "import { dazatTokens, presentCoreJourneyForSurface } from '@dazat/design-system';");
  }
  if (!next.includes('const journeyPresentation = journeyProgress')) {
  ...
  if (!next.includes('aria-label="Control Room canonical Journey projection"')) {
...
console.log('Phase 0.104 screen projection materialized for Rider, Driver and Control Room');
