...
export async function readCoreJourneyProgress(sessionToken: string, bookingId: string): Promise<CoreJourneyProgressProjection> {
  const response = await fetch(`${API_BASE_URL}/v1/bookings/${bookingId}/core-journey-progress`, {
